<script lang="ts">
  ...
  export default {
    ...
    computed: {
      noCodePhoneLabel () {
        const countryCode = `+${this.activeCountry?.dialCode}`

        return this.phone.startsWith(countryCode) ? this.phone.split(`${countryCode} `)[1] : this.phone
      },
      computedReferenceRef () {
        return this.referenceRef ?? this.root
      },
      activeCountry () {
...
<template>
  <div :class="['vue-tel-input', styleClasses, { disabled }]">
    <NatDropdown
      v-model="open"
    ...
    <!-- <pre class="fixed top-10 right-10 bg-white">{{ JSON.stringify(phoneObject, null, '  ') }}</pre> -->
  </div>
</template>
